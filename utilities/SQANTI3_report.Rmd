...
for (i in 1:length(p1.s.list)) {
  ...
  if (!is.null(p1.s.list[[i]])) {
    print(htmltools::tagList(ggplotly(p1.s.list[[i]] + theme(axis.title.x=element_blank()))%>%
...
```{r, eval = exists("pSTM")}
cat ('\n###',"Supported Transcript Models {.tabset .tabset-fade .tabset-dropdown}", '\n')
...
ggplotly(pSTM.s1 + theme(axis.title.x=element_blank()))%>%
  layout(showlegend = T)

ggplotly(pSTM_perc.s1 + theme(axis.title.x=element_blank()))%>%
  layout(showlegend = T)

cat ('\n####',"Structural Subcategories NIC and NNC", '\n')

ggplotly(pSTM.s2 + theme(axis.title.x=element_blank()))%>%
  layout(showlegend = T)

ggplotly(pSTM_perc.s2 + theme(axis.title.x=element_blank()))%>%
  layout(showlegend = T)
...
ggplotly(p8.s1 + theme(axis.title.x=element_blank())) 
```

```{r, eval = exists("p8.s2")}
cat ('\n####',"By Subcategory NIC/NNC", '\n')
ggplotly(p8.s2 + theme(axis.title.x=element_blank())) 
```

```{r, eval = exists("p8.s3")}
cat ('\n####',"By Subcategory Other", '\n')
ggplotly(p8.s3 + theme(axis.title.x=element_blank())) 
```
...
```{r, eval = exists("p9.s1")}
cat ('\n####',"By Subcategory FSM/ISM", '\n')
ggplotly(p9.s1 + theme(axis.title.x=element_blank())) 
```

```{r, eval = exists("p9.s2")}
cat ('\n####',"By Subcategory NIC/NNC", '\n')
ggplotly(p9.s2 + theme(axis.title.x=element_blank())) 
```

```{r, eval = exists("p9.s3")}
cat ('\n####',"By Subcategory Other", '\n')
ggplotly(p9.s3 + theme(axis.title.x=element_blank()) ) 
```
...
```{r, eval = exists("p.polyA_dist_subcat")}
...
```{r, eval = exists("df.polyA_subcat_MD")}
...
cat ('\n###'," Multi-exonic NIC {.tabset .tabset-fade .tabset-dropdown}", '\n')
```
...
cat ('\n###'," Multi-exonic NNC {.tabset .tabset-fade .tabset-dropdown}", '\n')
```
...
cat ('\n###',"CAGE Detected (table) {.tabset .tabset-fade .tabset-dropdown}", '\n')
...
cat ('\n####', "Only FSM", '\n')
ggplotly(new.1.FSM) %>%
  layout(margin = list(r = 130), textposition="top center")
cat ('\n')
```
...
### Quality Control Attributes Across Structural Categories {.tabset .tabset-fade  .tabset-dropdown}

```{r , eval=exists("p28")}
cat("#### Quality Control Attributes Across Structural Categories",'\n')
ggplotly(p28)%>%
  layout(margin = list(r = 130) , textposition="outside")  %>%
...
### TSS and TTS Support Percentages {.tabset .tabset-fade .tabset-dropdown}
...
### SJ Support Percentages {.tabset .tabset-fade .tabset-dropdown}

```{r, eval=exists("p28.a.SJ")}
cat("#### Isoforms all canonical", '\n') 
ggplotly(p28.a.SJ)%>%
  layout(margin = list(r = 130))%>%
  style(textposition="none")
cat ('\n')
...
```{r, eval=exists("p28.a.ratio.html")}
cat("#### ratio TSS", '\n') 
ggplotly(p28.a.ratio.html)%>%
